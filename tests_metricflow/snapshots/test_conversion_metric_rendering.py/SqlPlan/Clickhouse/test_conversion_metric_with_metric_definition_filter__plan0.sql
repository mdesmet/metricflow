...
FROM (
  ...
  SELECT
    ...
    , CAST(subq_18.__buys AS Nullable(DOUBLE PRECISION)) / CAST(NULLIF(subq_18.__visits, 0) AS Nullable(DOUBLE PRECISION)) AS visit_buy_conversion_rate_with_filter
  FROM (
              ...
              ON
                (
                  ...
                  (
                    ...
                    subq_9.metric_time__day > date_sub(day, 7, subq_12.metric_time__day)
                  )
                )
            ) subq_13
          ) subq_14
