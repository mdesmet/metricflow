...
SELECT
  ...
  , CAST(__buys AS Nullable(DOUBLE PRECISION)) / CAST(NULLIF(__visits, 0) AS Nullable(DOUBLE PRECISION)) AS visit_buy_conversion_rate_with_filter
FROM (
      ...
      ON
        (
          ...
          (
            ...
            subq_29.metric_time__day > date_sub(day, 7, subq_32.metric_time__day)
          )
        )
    ) subq_33
    GROUP BY
