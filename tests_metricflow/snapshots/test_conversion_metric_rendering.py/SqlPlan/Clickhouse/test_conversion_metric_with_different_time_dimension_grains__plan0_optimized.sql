...
SELECT
  CAST(MAX(subq_33.__buys_month) AS Nullable(DOUBLE PRECISION)) / CAST(NULLIF(MAX(subq_22.__visits), 0) AS Nullable(DOUBLE PRECISION)) AS visit_buy_conversion_rate_with_monthly_conversion
FROM (
    ...
    ON
      (
        ...
        (
          ...
          sma_28019_cte.metric_time__month > date_sub(month, 1, subq_28.metric_time__month)
        )
      )
  ) subq_29
) subq_33
