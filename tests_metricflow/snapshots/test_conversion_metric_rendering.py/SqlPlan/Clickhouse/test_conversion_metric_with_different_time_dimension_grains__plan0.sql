...
FROM (
  ...
  SELECT
    CAST(subq_16.__buys_month AS Nullable(DOUBLE PRECISION)) / CAST(NULLIF(subq_16.__visits, 0) AS Nullable(DOUBLE PRECISION)) AS visit_buy_conversion_rate_with_monthly_conversion
  FROM (
              ...
              ON
                (
                  ...
                  (
                    ...
                    subq_7.metric_time__month > date_sub(month, 1, subq_10.metric_time__month)
                  )
                )
            ) subq_11
          ) subq_12
