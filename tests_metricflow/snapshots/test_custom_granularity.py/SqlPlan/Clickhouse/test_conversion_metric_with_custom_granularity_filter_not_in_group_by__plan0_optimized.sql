...
SELECT
  CAST(MAX(subq_41.__buys) AS Nullable(DOUBLE PRECISION)) / CAST(NULLIF(MAX(subq_28.__visits), 0) AS Nullable(DOUBLE PRECISION)) AS visit_buy_conversion_rate_7days
FROM (
    ...
    ON
      (
        ...
        (
          ...
          subq_33.metric_time__day > date_sub(day, 7, subq_36.metric_time__day)
        )
      )
  ) subq_37
) subq_41
