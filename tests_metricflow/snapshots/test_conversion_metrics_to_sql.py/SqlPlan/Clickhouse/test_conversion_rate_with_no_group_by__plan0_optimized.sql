...
SELECT
  CAST(MAX(subq_33.__buys) AS Nullable(DOUBLE PRECISION)) / CAST(NULLIF(MAX(subq_22.__visits), 0) AS Nullable(DOUBLE PRECISION)) AS visit_buy_conversion_rate_7days
FROM (
    ...
    ON
      (
        ...
        (
          ...
          sma_28019_cte.metric_time__day > date_sub(day, 7, subq_28.metric_time__day)
        )
      )
  ) subq_29
) subq_33
